      ...
      >
        ...
        {[1, 2, 3, 4, 5].map((item) => {
          ...
          return <FaRegStar key={item} onClick={() => setRating(item)} />
                  ...
                  <Input placeholder="Please provide feedback here" onChange={(e) => setValue(e.target.value)} />
                  <Button sx={{mt: 2}} onClick={addChat}>Submit Feedback</Button>
         </Box>
        </Sheet>
      </Modal>
  );
...
export const ChatScreen = () => {
  ...
  return (
      <Box className='chat-screen'>
          <BasicModal open={open} handleClose={handleClose} addFeedback={addFeedback}/>
          <Messages />
          <Box className='input-container'>
              ...
              <Button className='send-button' onClick={endConversation}>End Conversation</Button>
          </Box>
    </Box>
  )
}
